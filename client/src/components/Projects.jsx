...
const Projects = () => {
  ...
  return (
    <section id="projects" className="py-16 sm:py-20 lg:py-24 bg-primary-50">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
        ...
        >
          <div className="inline-flex items-center space-x-2 bg-accent-100 text-accent-600 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium mb-4 sm:mb-6">
            <FaRocket className="text-accent-500" />
            <span>Featured Work</span>
          </div>
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-primary-800 mb-3 sm:mb-4">
            My <span className="text-gradient">Projects</span>
          </h2>
          <p className="text-base sm:text-lg text-primary-600 max-w-3xl mx-auto leading-relaxed px-4 sm:px-0">
        ...
        >
          {categories.map((category) => (
            <button
              ...
              className={`flex items-center space-x-1 sm:space-x-2 px-3 sm:px-4 lg:px-6 py-2 sm:py-2.5 rounded-full font-medium transition-colors text-xs sm:text-sm ${
        ...
        <div className="relative max-w-5xl mx-auto overflow-hidden">
          <div className="absolute left-3 sm:left-4 lg:left-1/2 lg:-translate-x-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-accent-200 via-primary-200 to-transparent" />

          <ul className="space-y-8 sm:space-y-10">
            {filteredProjects.map((project, index) => (
              <li key={project.id} className="relative">
                {/* Node */}
                <span className="absolute left-3 sm:left-4 lg:left-1/2 lg:-translate-x-1/2 top-2 w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full bg-accent-500 ring-2 sm:ring-4 ring-accent-100" />
                ...
                <motion.div
                  ...
                  className={`grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-10 items-start ${index % 2 === 0 ? 'lg:[&>*:first-child]:order-1' : ''}`}
                >
                  {/* Meta */}
                  <div className="pl-8 sm:pl-10 lg:pl-0">
                    <div className="flex items-center gap-2 mb-2">
                      <FaCalendarAlt className="text-primary-400" size={12} />
                      <span className="text-xs sm:text-sm text-primary-500">{project.year}</span>
                    ...
                    <div className="flex flex-wrap gap-1.5 sm:gap-2">
                      {project.technologies.map((tech, i) => (
                        <span key={i} className="text-xs px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full bg-primary-100 text-primary-700">
                          {tech}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="pl-8 sm:pl-10 lg:pl-0">
                    ...
                    <p className="text-primary-700 leading-relaxed mb-3 sm:mb-4 text-sm sm:text-base">
                      {project.description}
                    </p>
                    {project.features?.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-4 sm:mb-5">
                        {project.features.slice(0, 3).map((feature, i) => (
                          <span key={i} className="text-xs px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-md bg-accent-50 text-accent-700 border border-accent-100">
                            {feature}
                          </span>
                        ))}
                        {project.features.length > 3 && (
                          <span className="text-xs px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-md bg-primary-50 text-primary-700 border border-primary-100">
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                      {[...Array(5)].map((_, i) => (
                        <FaStar
                          key={i}
                          className={`text-xs sm:text-sm ${i < project.rating ? 'text-yellow-400' : 'text-primary-300'}`}
                        />
                      ))}
                      {project.liveUrl && (
                        <a
                          ...
                          className="ml-auto inline-flex items-center gap-1 sm:gap-2 text-accent-700 hover:text-accent-800 font-semibold text-sm sm:text-base"
                        >
        ...
        >
          <div className="bg-gradient-to-r from-accent-50 to-secondary-50 rounded-2xl sm:rounded-3xl p-6 sm:p-8 lg:p-10 border border-accent-100">
            <h3 className="text-2xl sm:text-3xl font-bold text-primary-800 mb-3">Ready to Start Your Next Project?</h3>
            <p className="text-primary-600 mb-4 sm:mb-6 text-base sm:text-lg max-w-2xl mx-auto">
              I'm always excited to work on new and challenging ideas.
            </p>
            <button
              onClick={() => document.querySelector('#contact').scrollIntoView({ behavior: 'smooth' })}
              className="btn-primary inline-flex items-center space-x-2 sm:space-x-3 text-base sm:text-lg px-6 sm:px-8 py-3 sm:py-4"
            >
              <FaEnvelope />
